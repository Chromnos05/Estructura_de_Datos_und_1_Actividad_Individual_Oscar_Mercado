"""
Control de Uso de una Sala de Cómputo - Implementación con STRUCT (Dataclass)
=============================================================================
Se usa @dataclass como equivalente de struct en Python.
Los datos son mutables (como un struct en C), pero sin lógica interna.
La lógica de negocio está separada en la clase SalaComputo.
"""
...
class ComputadorStruct:
    """Struct que almacena los datos de un computador.
    Equivalente a 'struct' en C/C++: agrupa datos relacionados sin métodos de negocio.
    """
    id_equipo: int
    ubicacion: str
...
class SalaComputo:
    """Sala de cómputo con un arreglo de structs (dataclasses)."""

    def __init__(self, cantidad: int):
        self.computadores: list[ComputadorStruct] = []
        for i in range(cantidad):
            self.computadores.append(
                ComputadorStruct(id_equipo=i + 1, ubicacion=f"Puesto {i + 1}")
            )
    ...
    def registrar_uso(self, id_equipo: int, usuario: str, hora_inicio: str) -> bool:
        ...
        for pc in self.computadores:
            if pc.id_equipo == id_equipo:
                if pc.disponible:
                    pc.disponible = False
                    pc.usuario_actual = usuario
                    pc.hora_inicio = hora_inicio
                    return True
                return False
        return False
    ...
    def mostrar_disponibles(self):
        ...
        for pc in disponibles:
            print(f"[PC-{pc.id_equipo:02d}] {pc.ubicacion} - Disponible")
    ...
    def buscar_por_usuario(self, usuario: str):
        ...
        for pc in encontrados:
            print(f"[PC-{pc.id_equipo:02d}] {pc.ubicacion} - Ocupado desde {pc.hora_inicio}")
        return encontrados
    ...
    def mostrar_todos(self):
        ...
        for pc in self.computadores:
            if pc.disponible:
                ...
                estado = f"Ocupado por {pc.usuario_actual} (desde {pc.hora_inicio})"
            print(f"[PC-{pc.id_equipo:02d}] {pc.ubicacion} - {estado}")
...
def main():
    print("============================================")
    print("  Sala de Cómputo - Implementación STRUCT")
    print("  (Usando @dataclass como struct)")
    print("============================================")
